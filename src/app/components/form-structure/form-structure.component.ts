...
export class FormStructureComponent {
  ...
  private assignSpreadsheetPayload(field: HandsontableField | null | undefined, fieldId: string, payload: any, version: number){
    if (!field) {
      return;
    }
  ...
  private outputStructureSnapshot(){
    ...
    // console.log(
    //   "[Form Structure]\n",
    //   JSON.stringify(this.droppedFields, null, 2)
    // );
  ...
  onRootSpreadsheetSubmit(fieldId: string, submission: { payload: any; version: number }){
    const field = this.droppedFields.find((f) => f.id === fieldId && this.isHandsontableField(f)) as HandsontableField | undefined;
