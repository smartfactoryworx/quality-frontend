...
export class HandsontableGridComponent implements AfterViewInit {
  ...
  @Output() gridSubmitted = new EventEmitter<any>();

  private internalHotId = `hot-grid-${HandsontableGridComponent.nextId++}`;
  @Input()
  set hotId(value: string | null | undefined) {
    if (typeof value === 'string' && value.trim().length) {
      this.internalHotId = value.trim();
    }
  }
  get hotId(): string {
    return this.internalHotId;
  }
      ...
      afterValidate: (isValid, value, row, prop, source) => {
        ...
        if (!isValid) {
          ...
          if (source === 'edit') {
            ...
            if (cur !== last) {
              this.ui.toast(`⚠️ Invalid value "${cur}"`, 'error');
              this.cellLastValues.set(key, cur);
            }
          }
        } else {
      ...
      cells: () => ({
        renderer: (inst, td, r, c, prop, value, cp) => {
          ...
          if (meta?.type === 'checkbox') {
            ...
            const tips: string[] = [];
            if (Array.isArray(meta?.allowedClickRoles) && meta.allowedClickRoles.length) {
              tips.push(`Approved Users: ${meta.allowedClickRoles.join(', ')}`);
            }
            ...
            if (meta?.isMandatory) td.style.outline = '2px solid #f57c00';
            else if (td.style.outline) td.style.outline = '';
          ...
          if (kind) {
            ...
            if (id) td.textContent = this.id2name[kind].get(id) ?? id;
              ...
              td.textContent = kind === 'equip' ? 'Select Equipment' : 'Select Status';
              td.style.color = '#999';
          ...
          if (meta.editor === 'datetime-local' || meta.editor === 'datetime') {
            td.style.borderRight = '3px solid #050506ff';
          ...
          if (meta?.isMandatory) {
            td.title = td.title || 'Mandatory Field';
            td.style.outline = '2px solid #f57c00';
          } else {
            if (td.title === 'Mandatory Field') td.removeAttribute('title');
            if (td.style.outline) td.style.outline = '';
          }

          const lbl = this.getAppliedLabel(r, c);
          if (lbl) td.title = lbl;
          ...
          if (meta?.validator && value && value !== '') {
            ...
            if (this.invalidCells.has(key)) {
              ...
              td.title = 'Invalid value - please correct';
            } else {
              td.style.border = '';
              td.style.backgroundColor = '';
              if (td.title === 'Invalid value - please correct') td.removeAttribute('title');
            }
          }
      ...
      afterChange: (changes, source) => {
        ...
        if (source === 'edit') {
          if (h) {
            ...
            this.dataChanged.emit(h.getData());
          }
        }
        ...
        changes.forEach(([row, prop, oldValue, newValue]) => {
          const h2 = this.hot;
          if (!h2 || typeof prop !== 'number') return;
          const meta: any = h2.getCellMeta(row, prop);
          if (meta?.type === 'checkbox') {
            const isApproval = Array.isArray(meta?.allowedClickRoles) && meta.allowedClickRoles.length > 0;
            if (newValue === true && isApproval) {
              h2.setCellMeta(row, prop, 'approvalStamp', stamp());
              h2.setCellMeta(row, prop, 'readOnly', true);
              setTimeout(() => h2.render(), 50);
            } else if (newValue === false && isApproval) {
              h2.removeCellMeta(row, prop, 'approvalStamp');
              setTimeout(() => h2.render(), 50);
            }
          }
        });
  ...
  markSelectionAsMeasurement() {
    this.forEachCell((r, c, h) => h.setCellMeta(r, c, 'fieldCategory', 'measurement'));
    h_render(this.hot);
  }
  markSelectionAsEquipment() {
    this.forEachCell((r, c, h) => h.setCellMeta(r, c, 'fieldCategory', 'equipment'));
    h_render(this.hot);
  }
  clearSelection() { this.clearSelectionInternal(); }
  loadDummyData() {
    const d = Array.from({ length: 8 }, (_, r) =>
  ...
  private rebuildMaps() {
    ...
    (this.equipmentOptions || []).forEach(o => {
      this.id2name.equip.set(String(o._id), o.name);
      this.name2id.equip.set(o.name, String(o._id));
    });
    (this.statusOptions || []).forEach(o => {
      this.id2name.status.set(String(o._id), o.name);
      this.name2id.status.set(o.name, String(o._id));
    });
  }

  // ----- utilities (SAFE hot getter) -----
  get hot(): Handsontable.Core | null {
  if (!this.hotId) return null;

  const inst = this.registerer.getInstance(this.hotId);
  if (!inst) return null;

  const destroyed = (inst as any)?.isDestroyed;
  ...
  private selBox() {
    const h = this.hot;
    if (!h) {
      return this.lastSel ?? { r1: 0, c1: 0, r2: 0, c2: 0 };
    }
    const live = h.getSelectedRangeLast();
    return live
      ? {
          r1: Math.min(live.from.row, live.to.row),
          c1: Math.min(live.from.col, live.to.col),
          r2: Math.max(live.from.row, live.to.row),
          c2: Math.max(live.from.col, live.to.col),
        }
  ...
  private forEachCell(cb: (r: number, c: number, h: Handsontable.Core) => void) {
    ...
    const ranges = h.getSelectedRange();
  ...
  private applyStyleToSelection(style: Partial<CSSStyleDeclaration>) {
    ...
    this.forEachCell((r, c, grid) => {
      const meta: any = grid.getCellMeta(r, c);
      const next = { ...(meta.style || {}), ...style };
      grid.setCellMeta(r, c, 'style', next);
    });
    h_render(h);
  }
  ...
  private convertNameToId(changes: (Handsontable.CellChange | null)[] | null) {
    ...
    for (const ch of changes) {
      ...
      if (kind) {
        const id = this.name2id[kind].get(newVal);
        if (id) ch[3] = id;
      }
    }
  }
  ...
  // validations & compare
  private requiredValidator = (value: any, cb: (ok: boolean) => void) =>
    cb(value != null && String(value).trim() !== '');
  ...
  private buildCompositeValidator(h: Handsontable.Core, r: number, c: number) {
    ...
    const composite = (v: any, cb: (ok: boolean) => void) => {
      if (meta['isMandatory']) {
        const okReq = v != null && String(v).trim() !== '';
        if (!okReq) return cb(false);
      }
  ...
  private applyValidationInternal(rule: 'date' | 'time' | 'integer' | 'float' | 'string') {
    ...
    this.forEachCell((r, c, grid) => {
      grid.setCellMeta(r, c, 'baseValidation', rule);
      if (rule === 'date') grid.setCellMeta(r, c, 'type', 'date');
      else if (rule === 'time') grid.setCellMeta(r, c, 'type', 'time');
      else if (rule === 'integer' || rule === 'float') grid.setCellMeta(r, c, 'type', 'numeric');
      else grid.setCellMeta(r, c, 'type', 'text');
      this.buildCompositeValidator(grid, r, c);
    });
  ...
  private applyCompareStatic() {
    ...
    if (!op) return;
    const rhs = prompt('Enter value:', '0');
    if (rhs == null) return;
    this.forEachCell((r, c, hh) => {
      hh.setCellMeta(r, c, 'compareRule', { type: 'static', op, rhs });
      this.buildCompositeValidator(hh, r, c);
    });
    h.validateCells(() => h.render());
  }
  ...
  private applyCompareBetween() {
    ...
    const min = prompt('Enter minimum:', '0'); if (min == null) return;
    const max = prompt('Enter maximum:', '10'); if (max == null) return;
    this.forEachCell((r, c, hh) => {
      hh.setCellMeta(r, c, 'compareRule', { type: 'between', min, max });
      this.buildCompositeValidator(hh, r, c);
    });
    h.validateCells(() => h.render());
  }
  ...
  private clearCompareRules() {
    ...
    this.forEachCell((r, c, hh) => {
      hh.removeCellMeta(r, c, 'compareRule');
      hh.removeCellMeta(r, c, 'validator');
      hh.removeCellMeta(r, c, 'allowInvalid');
      hh.setCellMeta(r, c, 'valid', true);
    });
    h.render();
  }

  private makeRequired() {
    const h = this.hot;
    if (!h) return;

    this.forEachCell((r, c, grid) => {
      grid.setCellMeta(r, c, 'isMandatory', true);
      grid.setCellMeta(r, c, 'validator', this.requiredValidator);
      grid.setCellMeta(r, c, 'allowInvalid', false);
    });
    h_render(h);
  }

  private clearRequired() {
    const h = this.hot;
    if (!h) return;

    this.forEachCell((r, c, grid) => {
      grid.removeCellMeta(r, c, 'isMandatory');
      grid.removeCellMeta(r, c, 'validator');
      grid.removeCellMeta(r, c, 'allowInvalid');
      grid.setCellMeta(r, c, 'valid', true);
    });
    h_render(h);
  }
  ...
  private submitGrid() {
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        if (meta?.isMandatory) {
          ...
          if (empty) {
            hasMandatoryEmpty = true;
            if (td) {
              td.style.border = '2px solid #d32f2f';
              td.title = 'This mandatory field is empty';
            }
          } else if (td) {
            td.style.border = '2px solid #f57c00';
    ...
    if (hasMandatoryEmpty) {
      this.ui.toast('⚠️ Complete all mandatory fields before submitting.', 'error');
      return;
    }

    h.validateCells((ok) => {
      if (!ok) {
        this.ui.toast('⚠️ Fix invalid cells before submit.', 'error');
        return;
      }
      ...
      this.gridSubmitted.emit(payload);
      this.ui.toast('✅ Submitted successfully!', 'success');
    });
  }

  private saveStructureInternal() {
    // your existing structure save logic can stay as-is
  ...
  private restoreInternal(payload: any) {
    ...
    h.loadData(pure);
    if (Array.isArray(payload.mergedCells) && payload.mergedCells.length > 0) {
      h.updateSettings({ mergeCells: payload.mergedCells });
    }
    if (Array.isArray(payload.rowHeights) || Array.isArray(payload.colWidths)) {
      h.updateSettings({ rowHeights: payload.rowHeights, colWidths: payload.colWidths });
    }
    ...
    payload.data.forEach((row: any[], r: number) => {
      row.forEach((cell: any, c: number) => {
        ...
        if (cell.isMandatory) {
          h.setCellMeta(r, c, 'isMandatory', true);
          h.setCellMeta(r, c, 'validator', this.requiredValidator);
        }
        switch (cell.controlType) {
          case 'dropdown': {
            const kind: Kind = cell.dataSource;
            const src = kind === 'equip'
              ? (this.equipmentOptions || []).map(o => o.name)
              : (this.statusOptions || []).map(o => o.name);
            h.setCellMeta(r, c, 'editor', 'dropdown');
            h.setCellMeta(r, c, 'source', src);
            h.setCellMeta(r, c, 'dropdownKind', kind);
            h.setCellMeta(r, c, 'strict', true);
          ...
          case 'checkbox':
            ...
            break;
          case 'datetime':
            h.setCellMeta(r, c, 'editor', 'date');
            h.setCellMeta(r, c, 'dateFormat', 'YYYY-MM-DD HH:mm');
            break;
          default:
            h.setCellMeta(r, c, 'type', 'text');
        }
        if (cell.fieldCategory) h.setCellMeta(r, c, 'fieldCategory', cell.fieldCategory);
        else h.removeCellMeta(r, c, 'fieldCategory');

        if (typeof cell.validationRange === 'string') {
          const parsed = this.parseValidationRange(cell.validationRange);
          if (parsed) {
            if (parsed.kind === 'static') {
              h.setCellMeta(r, c, 'compareRule', { type: 'static', op: parsed.op, rhs: parsed.rhs });
            } else {
              h.setCellMeta(r, c, 'compareRule', { type: 'between', min: parsed.min, max: parsed.max });
            }
  ...
  private getRowHeights(h: Handsontable.Core, rows: number) {
    const hs: number[] = [];
    for (let i = 0; i < rows; i++) {
      hs.push(h.getRowHeight(i) ?? 23);
    }
    return hs;
  }

  private getColWidths(h: Handsontable.Core, cols: number) {
    const ws: number[] = [];
    for (let i = 0; i < cols; i++) {
      ws.push(h.getColWidth(i) ?? 100);
    }
    return ws;
  }
...
function h_render(h: Handsontable.Core | null) {
  if (!h || (h as any).isDestroyed) return;
  h.validateCells?.(() => h.render());
}
