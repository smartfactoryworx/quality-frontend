...
export class AppStateService {
  private readonly localStorageKey = 'xintra-ng'; // Customize this key
  ...
  private applyThemeBackgroundSpecificChanges(background: any) {
    ...
    // html?.style.setProperty( '--primary-rgb', background.main);
    html?.style.setProperty( '--body-bg', background.main);
    html?.style.setProperty( '--dark-bg', background.secondary);
    html?.style.setProperty( '--light', background.accent );
  ...
  private applythemeSpecificChanges(theme: string) {
    let html = document.querySelector('html');
    html?.setAttribute('class', theme);  //setting theme style
  ...
  private applybackgroundImageSpecificChanges(backgroundImage: string) {
    let html = document.querySelector('html');        
    html?.setAttribute('bg-img', backgroundImage);
  }
